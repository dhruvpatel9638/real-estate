...
export default function LocationsSection() {
  ...
  return (
    <section ref={sectionRef} className="w-full bg-black text-white py-20 sm:py-24 md:py-28 px-4 sm:px-10 md:px-16 border-t border-neutral-900 overflow-hidden">
      <div className="max-w-7xl mx-auto flex flex-col gap-8 sm:gap-12">
        ...
        <div ref={tabsRef} className="flex items-center justify-between border-b border-neutral-800 pb-4" style={{ opacity: 0 }}>
          <span className="l1-t font-serif text-base sm:text-lg text-white">Flagship Office</span>
          <span className="l1 text-xs text-neutral-400">AHMEDABAD, GUJARAT</span>
        </div>

        {/* Content Grid */}
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-12 items-center">
          <div ref={contentRef} className="md:col-span-7 flex flex-col gap-4 sm:gap-6 order-2 md:order-1" style={{ opacity: 0 }}>
            <span className="loc-anim l1-t font-serif text-[#7a0c07] text-lg sm:text-xl">{currentLocation.building}</span>
            <h3 className="loc-anim h1 text-3xl sm:text-4xl md:text-5xl text-white uppercase">{currentLocation.title}</h3>
            <p className="loc-anim l1 text-xs text-neutral-400 leading-relaxed max-w-xl">{currentLocation.desc}</p>

            <div className="loc-anim flex flex-wrap gap-4 sm:gap-6 pt-2 sm:pt-4 text-xs l1">
            ...
            <div className="loc-anim pt-2 sm:pt-4">
              <a href={currentLocation.mapUrl} target="_blank" rel="noopener noreferrer"
                className="btn-pill border-white text-white hover:bg-white hover:text-black transition-all duration-300 inline-flex items-center gap-2 text-xs">
                <MapPin className="w-4 h-4" /><span>Show on map</span>
              </a>
            </div>
          </div>

          <div ref={imageRef} className="md:col-span-5 relative aspect-[4/5] rounded-xl overflow-hidden shadow-2xl bg-neutral-900 order-1 md:order-2" style={{ opacity: 0 }}>
