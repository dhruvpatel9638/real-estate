...
export default function About() {
  ...
  return (
    <section ref={sectionRef} id="about" className="w-full bg-white text-black py-28 px-6 sm:px-16 relative overflow-hidden">
      <div className="max-w-7xl mx-auto flex flex-col gap-16">
          ...
          {headingLines.map((line, i) => (
            <div key={i} className="overflow-hidden py-1 sm:py-2 -my-1 sm:-my-2">
              <div
                ...
                className="h1-t text-3xl sm:text-5xl leading-tight"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 my-4">
          {images.map((src, index) => (
            ...
            >
              <img
                src={src}
                alt={`Fame estate showcase ${index + 1}`}
                className="w-full h-full object-cover will-change-transform"
              />
              <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex items-end p-6">
                <span className="l1 text-white text-xs">FAME EXCLUSIVE</span>
              </div>
            </div>
          ))}
        </div>
        ...
        {/* Honors List */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 pt-10 border-t border-black/10">
          {honors.map((item, idx) => (
            <div
              key={idx}
              ...
              style={{ opacity: 0 }}
            >
              <span className="font-serif text-3xl font-semibold tracking-wide text-black border-l-2 border-[#7a0c07] pl-3">
                {item.title}
              </span>
